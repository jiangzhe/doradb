...
- Direction Hint: Prefer real production-shaped sources first: standard IO errors through IoError, malformed persisted bytes through DataIntegrityError, and spawn errors through RuntimeError. When no realistic source can be constructed, add a narrowly named cfg(test) variant to the applicable domain enum while keeping the hook and control state in the owning test module. Do not reintroduce a shared InjectedTestError or a generic injected marker. Establish a poison-with-source pattern that publishes the first Fatal poison reason while returning a report retaining the initiating source. Tests must assert source domain, outer Fatal context when applicable, and state or ownership cleanup.
