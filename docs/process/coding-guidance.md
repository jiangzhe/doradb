...
*   **Public Error Ownership**: Public `Error` is limited to public Doradb APIs, externally fixed trait adapters, constrained-carrier disclosure, and orchestration that combines public callbacks with engine failures. The callback-combining scope is managed DDL orchestration, callback-bearing statement methods, sequential and index mutation entry points and helpers, programmable transaction streams, and the shared cursor's callback/projection boundary. These carry `CallbackResult<_, E>` (or private `CE: From<Error>` settlement/cursor results); engine-only leaves and helpers retain their native result domains. Preserve `Engine(Error)` reports and `User(E)` payloads intact, without blanket `From<E>` or mandatory application trait bounds. Callback-free snapshot partition streams retain public `Result`. Reusable internal orchestration outside these boundaries remains typed.
...
*   **Test Contracts**: Every source-visible test in a changed Rust file needs literal documentation with exactly one nonempty `Purpose:` and `Expected:` field, above all other attributes. Keep both fields concise and intent-focused: `Purpose:` names the protected behavior and distinguishing scenario; `Expected:` states the semantic guarantee supported by assertions or an explicit oracle. Leave concrete fixture values and assertion details in the test body. Wrapped lines continue the preceding field. See [Unit Testing](unit-test.md#test-contracts-and-inventory) for writing guidance, syntax, inventory commands, and gradual adoption.
*   **Semantic Test Review**: After mechanical gates pass, connect changed tests and related inventory entries to their invariant, input boundaries, independent expected results, actual assertions, and state/schedule setup. Record seeds and synchronization predicates where relevant; a seed alone does not establish reproducibility. Review nearby tests and shared procedures even when contract wording differs. Exact contract matches only identify review candidates. Before consolidating or deleting tests, account for features/backends, public/component contracts, owner/lifecycle differences, and independent oracles. Preserve named cases and case-specific diagnostics, and record why intentional overlap remains.
