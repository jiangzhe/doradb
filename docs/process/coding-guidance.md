...
*   **Test Contracts**: Every source-visible test in a changed Rust file needs literal documentation with exactly one nonempty `Purpose:` and `Expected:` field, above all other attributes. `Purpose:` names the protected behavior and distinguishing scenario; `Expected:` names observable results that assertions or an explicit oracle actually prove. Wrapped lines continue the preceding field. See [Unit Testing](unit-test.md#test-contracts-and-inventory) for syntax, inventory commands, and gradual adoption.
