...
pub struct LgcPlan {
    ...
    /// maximum global id of columns.
    /// It is used to generate new columns in optimization phase.
    pub max_cid: GlobalID,
}
...
impl LgcPlan {
    ...
    pub fn new<C: Catalog>(
        ...
        let builder = LgcBuilder::new(catalog, default_schema)?;
        builder.build(query)
    }
...
pub(crate) mod tests {
    ...
    fn test_plan_build_select_row() {
        ...
        for sql in vec![
            ...
            let builder = LgcBuilder::new(&cat, "default").unwrap();
    ...
    fn test_plan_build_select_table() {
        ...
        for (sql, n_cols, shape) in vec![
            ...
            let builder = LgcBuilder::new(&cat, "tpch").unwrap();
            let qr = parse_query(MySQL(sql)).unwrap();
            let plan = match builder.build(&qr) {
                Ok(plan) => plan,
                Err(e) => {
    ...
    fn test_plan_build_subquery() {
        ...
        let builder = LgcBuilder::new(&cat, "j").unwrap();
        let qr = parse_query(MySQL(sql)).unwrap();
        assert!(builder.build(&qr).is_err())
    }
    }
    ...
    #[inline]
    pub(crate) fn table_map<C: Catalog>(
        cat: &C,
    ...
    pub(crate) fn assert_j_plan1<C: Catalog, F: FnOnce(&str, LgcPlan)>(cat: &C, sql: &str, f: F) {
        let plan = build_plan(cat, "j", sql);
        f(sql, plan)
    }

    pub(crate) fn assert_j_plan2<C: Catalog, F: FnOnce(&str, LgcPlan, &str, LgcPlan)>(
        cat: &C,
        sql1: &str,
        sql2: &str,
        f: F,
    ...
    pub(crate) fn build_plan<C: Catalog>(cat: &C, schema_name: &str, sql: &str) -> LgcPlan {
        let builder = LgcBuilder::new(cat, schema_name).unwrap();
        let qr = parse_query(MySQL(sql)).unwrap();
        builder.build(&qr).unwrap()
    }
    ...
    pub(crate) fn get_subq_filt_expr(subq: &Subquery) -> Vec<doradb_expr::ExprKind> {
        let mut cfe = CollectFiltExpr(vec![]);
        let _ = subq.root.walk(&mut cfe);
        cfe.0
    }

    pub(crate) fn get_table_filt_expr(subq: &Subquery) -> Vec<doradb_expr::ExprKind> {
    ...
    struct CollectTableFiltExpr(Vec<doradb_expr::ExprKind>);
    ...
    struct CollectFiltExpr(Vec<doradb_expr::ExprKind>);
