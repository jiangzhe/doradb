...
impl<'a, 'op, 'r, 'ctx> IndexMutator<'a, 'op, 'r, 'ctx> {
    ...
    pub(super) async fn mutate_index_candidate<F, E>(
        &mut self,
    ...
    async fn mutate_hot_index_candidate<F, E>(
        &mut self,
        candidate: &BoundIndexCandidate<'_>,
        page_id: PageID,
        row_buffer: &mut LazyRowBuffer,
        outcome: &mut TableMutationOutcome,
        mutate_row: &mut F,
    ) -> CallbackResult<CandidateProgress, E>
    where
        F: for<'row> FnMut(&mut LazyRow<'row>) -> CallbackResult<RowMutation, E>,
    {
    ...
    async fn mutate_cold_index_candidate<F, E>(
        &mut self,
        candidate: &BoundIndexCandidate<'_>,
    ...
    async fn mutate_owned_hot_index_candidate<F, E>(
        &mut self,
        candidate: &BoundIndexCandidate<'_>,
        ...
        outcome: &mut TableMutationOutcome,
        mutate_row: &mut F,
    ) -> CallbackResult<Option<WriteIndexKeySet<'op>>, E>
    where
        F: for<'row> FnMut(&mut LazyRow<'row>) -> CallbackResult<RowMutation, E>,
    {
        let accessor = self.accessor;
...
mod tests {
    ...
    fn test_table_index_mutate_mvcc_unique_driver_change_moves_frozen_row() {
        smol::block_on(async {
            ...
            setup
                .table_insert_mvcc(table_id, vec![Val::from("old")])
                .await
                .unwrap();
    ...
    fn test_table_index_mutate_mvcc_non_unique_forward_moves_skip_self_entries() {
        smol::block_on(async {
            ...
            let mut setup = session.begin_trx().unwrap();
            let rows = (0..5i32)
                .map(|id| vec![Val::from(id), Val::from(id)])
                .collect();
            setup.table_insert_batch_mvcc(table_id, rows).await.unwrap();
            setup.commit().await.unwrap();
