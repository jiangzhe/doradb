...
use crate::io::{
    ...
    FileSyncer, FreeListWithFactory, IocbRawPtr, PageBuf, SparseFile, AIO,
};
use crate::trx::{
...
/// much CPU and slow down the log writer.
///
        ...
        for (idx, partition) in self.log_partitions.iter().enumerate() {
            let (tx, rx) = flume::unbounded();
            {
                ...
                let handle = if self.config.log_drop {
                    ...
                    builder.spawn(move || partition.io_loop(&self.config, tx))
                }
                .unwrap();
                *partition.io_thread.lock() = Some(handle);
            }
            {
                let thread_name = format!("Log-Sync-{}", idx);
                let partition = &**partition;
                let handle = thread::Builder::new()
                    .name(thread_name)
                    .spawn(move || partition.sync_loop(&self.config, rx))
                    .unwrap();
...
impl Drop for TransactionSystem {
    ...
    fn drop(&mut self) {
        ...
        // wait for IO thread and sync thread to quit.
        for partition in log_partitions {
            let io_thread = { partition.io_thread.lock().take().unwrap() };
...
impl TrxSysConfig {
    ...
    pub fn build_static(self) -> &'static TransactionSystem {
        ...
        trx_sys.start_io_and_sync_threads();
        trx_sys
    }
}
...
struct LogPartition {
    ...
    // Standalone thread to handle IO requests, wait IO responses.
    io_thread: Mutex<Option<JoinHandle<()>>>,
    // Standalone thread to do fsync().
...
impl LogPartition {
    ...
    #[inline]
    fn update_sync_stats(&self, sync_count: usize, sync_nanos: usize) {
        self.stats
            .sync_count
    ...
    fn io_loop_noop(&self, config: &TrxSysConfig) {
        ...
        loop {
            ...
            if !io_reqs.is_empty() {
                ...
                io_reqs.clear();
                sync_groups.clear();
                self.update_io_stats(trx_count, commit_count, log_bytes, 0, 0, 0, 0);
            }
            if shutdown {
                return;
            }
    ...
    #[inline]
    fn io_loop(&self, config: &TrxSysConfig, sync_chan: Sender<SyncMessage>) {
        let io_depth = config.io_depth_per_log;
        ...
        loop {
            ...
            if !written.is_empty() {
                let _ = sync_chan.send(SyncMessage::Group(written));
            }
    ...
    fn sync_loop(&self, config: &TrxSysConfig, sync_chan: Receiver<SyncMessage>) {
        let syncer = {
            self.group_commit
                .0
                .lock()
        ...
        while let Ok(msg) = sync_chan.recv() {
            match msg {
                SyncMessage::Group(sync_groups) => {
                    let max_cts = sync_groups.last().unwrap().max_cts;
                    ...
                    let sync_nanos = start.elapsed().as_nanos() as usize;

                    self.persisted_cts.store(max_cts, Ordering::SeqCst);

                    // put IO buffer back into free list.
                    for mut sync_group in sync_groups {
                    ...
                    self.update_sync_stats(1, sync_nanos);
                }
                SyncMessage::Switch => todo!(),
            }
        }
    }
}
