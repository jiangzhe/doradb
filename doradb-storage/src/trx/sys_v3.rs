...
pub struct TransactionSystem {
    ...
    group_commit_v3: Box<[CachePadded<Mutex<GroupCommit>>]>,
    ...
    redo_loggers_v3: Box<[Mutex<Box<dyn RedoLogger>>]>,
...
impl TransactionSystem {
    ...
    pub fn new_static(
        partitions: usize,
        redo_loggers: Vec<Mutex<Box<dyn RedoLogger>>>,
    ) -> &'static Self {
        let sys = Self::new(partitions, redo_loggers);
    ...
    #[inline]
    fn group_commit_partition_idx(&self, input: u64) -> usize {
        let mut s = DefaultHasher::new();
        input.hash(&mut s);
        s.finish() as usize % self.group_commit_v3.len()
    }
    ...
    #[inline]
    pub fn commit_v3(&self, trx: ActiveTrx) {
        let partition_idx = self.group_commit_partition_idx(trx.sts);
        ...
        // start group commit
        let mut group_commit_g = self.group_commit_v3[partition_idx].lock();
        ...
        match group_commit_g.groups.len() {
            0 => self.act_as_single_trx_leader(precommit_trx, partition_idx, group_commit_g),
            1 => {
                ...
                match &mut curr_group.kind {
                    ...
                    CommitGroupKind::Processing => {
                        self.act_as_next_group_leader(precommit_trx, partition_idx, group_commit_g)
                    }
                    kind => {
    ...
    #[inline]
    pub fn start_gc_thread_v3(&'static self) {
        ...
        *gc_thread_g = Some(GCThread(handle));
        ...
        for partition in &self.group_commit_v3 {
            let mut group_commit_g = partition.lock();
            debug_assert!(group_commit_g.gc_chan.is_none());
            group_commit_g.gc_chan = Some(tx.clone());
        }
    }
    ...
    #[inline]
    pub fn group_commit_stats_v3(&self) -> Vec<GroupCommitStats> {
        let mut res = Vec::with_capacity(self.group_commit_v3.len());
        for partition in &self.group_commit_v3[..] {
            res.push(partition.lock().stats);
        }
        res
    }
    ...
    fn commit_single_trx(
        ...
        if let Some(redo_bin) = trx.redo_bin {
            let mut logger_g = self.redo_loggers_v3[partition_idx].lock();
            log_bytes += logger_g.write(trx.cts, redo_bin);
            logger_g.flush();
        }
    ...
    fn act_as_single_trx_leader(
        ...
        let (cts, log_bytes) = self.commit_single_trx(precommit_trx, partition_idx, gc_chan);
    ...
    fn act_as_next_group_leader(
        ...
        let (persisted_cts, trx_count, log_bytes) = match kind {
            CommitGroupKind::Single(trx) => {
                let (cts, log_bytes) = self.commit_single_trx(trx, partition_idx, gc_chan);
                (cts, 1, log_bytes)
            }
            CommitGroupKind::Multi(trx_list) => {
                let count = trx_list.len();
                let (cts, log_bytes) = self.commit_multi_trx(trx_list, partition_idx, gc_chan);
                (cts, count, log_bytes)
            }
    ...
    #[inline]
    fn new(partitions: usize, redo_loggers: Vec<Mutex<Box<dyn RedoLogger>>>) -> Self {
        assert!(partitions == redo_loggers.len());
        let partitions: Vec<_> = (0..partitions)
            .map(|_| CachePadded::new(Mutex::new(GroupCommit::new())))
            .collect();
        TransactionSystem {
            ts: CachePadded::new(AtomicU64::new(MIN_SNAPSHOT_TS)),
            min_active_sts: CachePadded::new(AtomicU64::new(MIN_SNAPSHOT_TS)),
            group_commit_v3: partitions.into_boxed_slice(),
            ...
            redo_loggers_v3: redo_loggers.into_boxed_slice(),
...
mod tests {
    ...
    impl RedoLogger for NoOpLogger {
        fn write(&mut self, cts: TrxID, redo_bin: crate::prelude::redo::RedoBin) -> usize {
            0
        }
        fn flush(&mut self) {}
    }
    ...
    fn test_transaction_system() {
        let trx_sys = TransactionSystem::new_static(1, vec![Mutex::new(Box::new(NoOpLogger))]);
        {
