...
pub struct TransactionSystem {
    ...
    group_commit_v5: CachePadded<Mutex<GroupCommit>>,
...
impl TransactionSystem {
    ...
    #[inline]
    pub async fn commit_v5(&self, trx: ActiveTrx) {
        ...
        // start group commit
        let mut group_commit_g = self.group_commit_v5.lock();
    ...
    #[inline]
    pub fn group_commit_stats_v5(&self) -> GroupCommitStats {
        self.group_commit_v5.lock().stats
    }
    ...
    fn act_as_single_trx_leader(
        ...
        let (_, log_bytes) = self.commit_single_trx(precommit_trx, gc_chan);
        ...
        // new transaction may join the group which is not started, or form a new group and wait.
        {
            ...
            let _ = group_commit_g.groups.pop_front().unwrap();
        }
    }
    ...
    async fn act_as_next_group_leader(
        ...
        let (_, trx_count, log_bytes) = match kind {
    ...
    fn new(log_len_threshold: usize) -> Self {
        TransactionSystem {
            ...
            group_commit_v5: CachePadded::new(Mutex::new(GroupCommit::new(log_len_threshold))),
...
mod tests {
    ...
    fn test_transaction_system() {
        let trx_sys = TransactionSystem::new_static(LOG_LEN_THRESHOLD);
        {
            let trx = trx_sys.new_trx();
            smol::block_on(trx_sys.commit_v5(trx));
        }
        std::thread::spawn(|| {
            let trx = trx_sys.new_trx();
            smol::block_on(trx_sys.commit_v5(trx));
    ...
    fn test_trx_sys_group_commit() {
        ...
        trx_sys.start_gc_thread();
        {
            ...
            smol::block_on(trx_sys.commit_v5(new_trx));
        }
