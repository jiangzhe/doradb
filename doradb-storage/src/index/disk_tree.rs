...
impl<'a, M: MutableCowFile> RewriteAllocationGuard<'a, M> {
    ...
    fn allocate_block(&mut self) -> RuntimeResult<BlockID> {
        let block_id = self
            ...
            .attach("operation=allocate_secondary_disk_tree_block")?;
...
impl<'a, F: DiskTreeSpec> DiskTree<'a, F> {
    ...
    #[inline]
    async fn read_node(&self, block_id: BlockID) -> RuntimeOrFatalResult<ValidatedDiskTreeNode<F>> {
        let guard = self
            .runtime
            .disk_pool
            ...
            .attach_with(|| {
                format!(
                    "operation=read_secondary_disk_tree_node, file={}, block_id={block_id}",
                    self.file_kind()
                )
            })?;
    ...
    async fn lookup_encoded_entry(&self, key: &[u8]) -> RuntimeOrFatalResult<Option<LogicalEntry>> {
                ...
                for idx in (0..node.count()).rev() {
                    ...
                    if child_block_id == SUPER_BLOCK_ID {
                        return Err(Report::new(DataIntegrityError::InvalidPayload)
                            ...
                            .change_context(RuntimeError::IndexAccess)
                            .attach("operation=collect_secondary_disk_tree_reachable_blocks").into());
                    }
    ...
    async fn absorb_rewrite_siblings(
        ...
        entries: Vec<RewriteEntry>,
        range_upper_fence: Option<&[u8]>,
    ) -> RuntimeOrFatalResult<Vec<RewriteEntry>> {
        if entries.len() < 2 {
            return Ok(entries);
        }
    ...
    async fn repack_rewrite_window(
        ...
        if height == 0 {
            ...
            return self
                .pack_leaf_rewrite_entries(&entries, upper_fence)
                .map_err(Into::into);
        }
    ...
    async fn rewrite_entry_payload(
        &self,
        entry: &RewriteEntry,
    ) -> RuntimeOrFatalResult<RewriteEntryPayload> {
        match entry {
    ...
    async fn branch_entry_payload(
        &self,
        entry: &BranchEntry,
    ) -> RuntimeOrFatalResult<BranchEntryPayload> {
        ...
        if node.height() != usize::from(entry.height) {
            return Err(Report::new(DataIntegrityError::InvalidPayload)
                .attach(format!(
                    "file={}, block=secondary_disk_tree, block_id={}, expected_height={}, actual_height={}",
                    self.file_kind(),
                    entry.block_id,
                    entry.height,
                    node.height()
                ...
                .change_context(RuntimeError::IndexAccess)
                .attach("operation=decode_secondary_disk_tree_node").into());
        }
        if node.is_leaf() {
            let mut entries = Vec::with_capacity(node.count());
            for idx in 0..node.count() {
    ...
    async fn retarget_rewrite_entry_upper_fence<M: MutableCowFile>(
        ...
        entry: RewriteEntry,
        upper_fence: Option<Vec<u8>>,
    ) -> RuntimeOrFatalResult<RewriteEntry> {
        match entry {
    ...
    async fn materialize_rewrite_entries<'m, M: MutableCowFile + 'm>(
        ...
        entries: Vec<RewriteEntry>,
        create_ts: TrxID,
    ) -> RuntimeOrFatalResult<Vec<BranchEntry>> {
    ...
    async fn write_node_block<M: MutableCowFile>(
        ...
        mutable_file
            ...
            .attach("operation=write_node, storage=secondary_disk_tree")
    }
}
...
impl DiskTreeNodeCursorState {
    ...
    #[inline]
    fn file_kind<F: DiskTreeSpec>(runtime: &DiskTreeRuntime<F>) -> FileKind {
        runtime.file_kind
    }

    /// Seek to the first leaf that can contain `key`.
    #[inline]
    pub(crate) async fn seek(&mut self, key: &[u8]) -> RuntimeOrFatalResult<()> {
        self.stack.clear();
        self.seek_key.clear();
        let Some(root_block_id) = self.root_block_id else {
    ...
    pub(crate) async fn next_leaf<F: DiskTreeSpec>(
        &mut self,
        runtime: &DiskTreeRuntime<F>,
        disk_pool_guard: &PoolGuard,
    ) -> RuntimeOrFatalResult<Option<DiskTreeLeaf<F>>> {
        while let Some(block_id) = self.stack.pop() {
            let guard = Self::read_node(runtime, disk_pool_guard, block_id).await?;
            let node = guard.node();
            if node.is_leaf() {
                ...
                return Ok(Some(DiskTreeLeaf {
                    file_kind: Self::file_kind(runtime),
                    block_id,
                    guard,
                    start_idx,
                }));
                ...
                node.lower_bound_child_entry_idx(&self.seek_key)
                    .ok_or_else(|| {
                        Report::new(DataIntegrityError::InvalidPayload)
                            ...
                            .change_context(RuntimeError::IndexAccess)
                            .attach("operation=scan_secondary_disk_tree_leaf")
                    })?
            } else {
                if node.lower_fence_value().is_deleted() {
                    return Err(Report::new(DataIntegrityError::InvalidPayload)
                        .attach(format!(
                            "file={}, block=secondary_disk_tree, block_id={block_id}, lower-fence child is delete-marked",
                            Self::file_kind(runtime)
                        ))
                        .change_context(RuntimeError::IndexAccess)
                        .attach("operation=scan_secondary_disk_tree_leaf").into());
                }
    ...
    #[inline]
    async fn read_node<F: DiskTreeSpec>(
        runtime: &DiskTreeRuntime<F>,
        disk_pool_guard: &PoolGuard,
        block_id: BlockID,
    ) -> RuntimeOrFatalResult<ValidatedDiskTreeNode<F>> {
        let guard = runtime
            .disk_pool
            .read_validated_block(
                runtime.file_kind,
...
impl<'a, F: DiskTreeSpec> DiskTreeNodeCursor<'a, F> {
    ...
    fn new(tree: &DiskTree<'a, F>) -> Self {
        Self {
            ...
            state: DiskTreeNodeCursorState::new(tree.root_block_id),
        }
    }

    /// Seek to the first leaf that can contain `key`.
    #[inline]
    pub(super) async fn seek(&mut self, key: &[u8]) -> RuntimeOrFatalResult<()> {
        self.state.seek(key).await
    }

    /// Return the next persisted leaf and first slot that may match the seek key.
    pub(super) async fn next_leaf(&mut self) -> RuntimeOrFatalResult<Option<DiskTreeLeaf<F>>> {
        self.state
            .next_leaf(self.runtime, self.disk_pool_guard)
            .await
    }
}
...
impl<'a> UniqueDiskTree<'a> {
    ...
    #[inline]
    pub(crate) async fn lookup(&self, key: &[Val]) -> RuntimeOrFatalResult<Option<RowID>> {
    ...
    #[inline]
    pub(crate) async fn lookup_encoded(&self, key: &[u8]) -> RuntimeOrFatalResult<Option<RowID>> {
    ...
    pub(crate) fn scan_candidate_stream<'r>(
        &self,
        range: &'r KeyRange,
    ) -> UniqueDiskTreeCandidateStream<'a, 'r> {
        UniqueDiskTreeCandidateStream::new(self.node_cursor(), range)
    }
...
impl<'a> NonUniqueDiskTree<'a> {
    ...
    #[inline]
    pub(crate) async fn contains_exact(
        &self,
        key: &[Val],
        row_id: RowID,
    ...
    #[inline]
    pub(crate) async fn contains_exact_encoded(&self, key: &[u8]) -> RuntimeOrFatalResult<bool> {
    ...
    pub(crate) fn scan_candidate_stream<'r>(
        &self,
        range: &'r KeyRange,
    ) -> NonUniqueDiskTreeCandidateStream<'a, 'r> {
        NonUniqueDiskTreeCandidateStream::new(self.node_cursor(), range)
    }
...
#[inline]
pub(super) fn unpack_row_id_from_exact_key(key: &[u8]) -> DataIntegrityResult<RowID> {
...
/// Validate the durable payload shape of already-encoded non-unique exact keys.
fn validate_non_unique_exact_key_payloads(
    entries: &[NonUniqueDiskTreeEncodedExact<'_>],
) -> DataIntegrityResult<()> {
    for entry in entries {
...
mod tests {
    ...
    trait UniqueDiskTreeBatchWriterTestExt {
        fn batch_put(&mut self, entries: &[UniqueDiskTreePut<'_>]) -> RuntimeResult<()>;
    ...
    trait NonUniqueDiskTreeBatchWriterTestExt {
        fn batch_insert(&mut self, entries: &[NonUniqueDiskTreeExact<'_>]) -> RuntimeResult<()>;

        fn batch_exact_delete(
            &mut self,
    ...
    async fn collect_node_summaries<F: DiskTreeSpec>(
        tree: &DiskTree<'_, F>,
    ) -> RuntimeOrFatalResult<Vec<NodeSummary>> {
    ...
    async fn collect_leaf_blocks<F: DiskTreeSpec>(
        tree: &DiskTree<'_, F>,
    ) -> RuntimeOrFatalResult<Vec<LeafBlock>> {
        let Some(root_block_id) = tree.root_block_id else {
            return Ok(Vec::new());
        };
    ...
    fn test_disk_tree_branch_fences_enable_common_prefix() {
        smol::block_on(async {
            ...
            let mut stream = UniqueDiskTreeCandidateStream::new(tree.node_cursor(), &range);
            let streamed = drain_row_ids(&mut stream).await;
            assert_eq!(
                streamed,
                (129usize..275)
    ...
    fn test_encoded_batch_writer_apis() {
        smol::block_on(async {
            ...
            let malformed_exact = [0u8; ROW_ID_SIZE - 1];
            {
                ...
                assert!(
                    format!("{err:?}").contains("operation=stage_secondary_disk_tree_insert_batch")
                );
            }
            {
                let mut writer = non_unique_tree.batch_writer(&mut mutable, TrxID::new(4));
                ...
                assert!(
                    format!("{err:?}").contains("operation=stage_secondary_disk_tree_delete_batch")
                );
            }
            {
                let mut writer = non_unique_tree.batch_writer(&mut mutable, TrxID::new(4));
