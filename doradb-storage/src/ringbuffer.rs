...
/// lower 4-byte is used to record offset.
// const RB_OFF_MASK: u64 = u32::MAX as u64;
/// highest bit is used to determine whether the buffer is wrapped.
/// In the wrap case, producer should wait for consumer to advance the read index and make free space.
// const WRAP_LOCK_BIT: u64 = 1 << 63;
/// Maximum value of ring buffer offset.
// const RB_OFF_MAX: u64 = u64::MAX & !WRAP_LOCK_BIT;
/// Mask of wrap counter.
// const WRAP_COUNTER: u64 = RB_OFF_MAX & !RB_OFF_MASK;
...
// #[inline(always)]
// fn incr_wrap(offset: u64) -> u64 {
//     (offset + 0x1_0000_0000u64) & WRAP_COUNTER
// }
...
mod tests {
    ...
    #[test]
    fn test_ring_buffer() {
        let rb = RingBuffer::<256>::new();
        let res1 = rb.produce(100);
        assert!(res1.next == 0);
        assert!(res1.target == 100);
        assert!(matches!(res1.area, AreaMut::Single(_)));
        let res2 = rb.produce(100);
        assert!(res2.next == 100);
        assert!(res2.target == 200);
        assert!(matches!(res2.area, AreaMut::Single(_)));
        // we need to drop in order, otherwise deadlock...
        drop(res1);
        drop(res2);
    }
}
