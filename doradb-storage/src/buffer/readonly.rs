...
/// Lease that keeps one physical readonly-cache key blocked while a CoW write
/// is queued or in flight.
///
/// Unlike the evictable pool writeback path, this readonly CoW path does not
/// queue reads behind writes. A production readonly read should reach only
/// blocks published through an active root, so a miss against a write-blocked
/// physical key is surfaced as an internal protocol error instead of waiting.
pub(crate) struct ReadonlyWriteLease {
    pool: QuiescentGuard<ReadonlyBufferPool>,
    key: BlockKey,
}

impl ReadonlyWriteLease {
    #[inline]
    fn new(pool: QuiescentGuard<ReadonlyBufferPool>, key: BlockKey) -> Self {
        Self { pool, key }
    }
}

impl fmt::Debug for ReadonlyWriteLease {
    #[inline]
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("ReadonlyWriteLease")
            .field("key", &self.key)
            .finish_non_exhaustive()
    }
}

impl Drop for ReadonlyWriteLease {
    #[inline]
    fn drop(&mut self) {
        if let Entry::Occupied(occ) = self.pool.inflights.entry(self.key)
            && matches!(occ.get(), InflightBlockState::WriteBlocked)
        {
            occ.remove();
        }
    }
...
impl ReadonlyBufferPool {
    ...
    /// Creates a new clone root for keeping this readonly pool alive.
    ///
    /// This has the same lifecycle-boundary semantics as
    /// [`BufferPool::create_base_guard`]: every call acquires a pool-global
    /// quiescent keepalive and allocates a fresh outer `Arc` root.
    #[inline]
    ...
    #[inline]
    async fn reserve_frame_id_for_load(&self) -> LifecycleResult<PageID> {
        loop {
    ...
    pub(crate) fn invalidate_block(
        ...
        self.invalidate_key(guard, &BlockKey::new(file_id, block_id))
    }
    ...
    #[inline]
    fn rollback_frame(&self, frame_id: PageID, mut page_guard: PageExclusiveGuard<Page>) {
...
impl QuiescentGuard<ReadonlyBufferPool> {
    ...
    fn read_diagnostic(
        ...
        format!(
            "buffer_pool_type=readonly, buffer_pool_role={}, operation={operation}, file_kind={file_kind}, file_id={}, block_id={block_id}",
    ...
    async fn join_or_start_inflight_load(
        ...
        let inflight = match self.inflights.entry(key) {
            ...
            Entry::Occupied(occ) => match occ.get() {
                ...
                InflightBlockState::WriteBlocked => {
                    return Err(
                        Report::new(InternalError::ReadonlyWriteBlocked).attach(format!(
        ...
        // The reservation and submitted IO may outlive this caller, so clone
        // the caller's root into the task-owned arena guard. Creating a new
        // base root here would touch the pool-global quiescent counter on every
        // cache miss and would discard the caller's session sharding domain.
        let task_arena = self.arena.arena_guard(guard.clone());
        match ReadonlyPageReservation::reserve_page(self, task_arena).await {
...
impl ReadonlyPageReservation {
    ...
    async fn reserve_page(
        ...
        arena: ArenaGuard,
    ) -> LifecycleResult<(PageID, PageExclusiveGuard<Page>)> {
        loop {
            let frame_id = pool.reserve_frame_id_for_load().await?;
            let Some(mut page_guard) = arena.try_lock_page_exclusive(frame_id) else {
                let frame_kind = arena.frame_kind(frame_id);
                assert!(
                    frame_kind == FrameKind::Uninitialized,
                    "readonly reserved frame latch contention expected uninitialized frame: frame_id={frame_id}, actual_kind={frame_kind:?}"
                );
...
///
/// The caller supplies its existing guard root. This path can run once per CoW
/// block write and must not create an independent base root for invalidation.
#[inline]
pub(crate) fn begin_write_barrier(
    ...
    file_id: FileID,
    block_id: BlockID,
) -> InternalResult<ReadonlyWriteLease> {
    pool.validate_guard(guard);
    let key = BlockKey::new(file_id, block_id);
    {
    ...
    let _ = pool.invalidate_key(guard, &key);
...
pub(crate) mod tests {
    ...
    impl ControlledWriteHook {
        fn for_page(fd: RawFd, block_id: BlockID) -> Self {
            ControlledWriteHook {
                inner: Arc::new(ControlledWriteHookInner {
                    fd,
                    offset: usize::from(block_id) * PAGE_SIZE,
                    calls: AtomicUsize::new(0),
                    start_ev: Event::new(),
                    released: AtomicBool::new(false),
                    release_ev: Event::new(),
    ...
    impl StorageBackendTestHook for ControlledWriteHook {
        fn on_submit(&self, op: StorageBackendOp) {
            if self.matches(op) {
                self.inner.calls.fetch_add(1, Ordering::SeqCst);
                self.inner.start_ev.notify(usize::MAX);
    ...
    fn test_readonly_miss_and_invalidation_reuse_caller_base_guard() {
        smol::block_on(async {
            let (_temp_dir, fs) = build_test_fs();
            let table_id = test_user_table_id(121);
            let table_file = fs
                .create_table_file(table_id, make_metadata(), false)
                .unwrap();
            let table_file = commit_table_file(&fs, table_file).await;
            let block_id = test_block_id(19);
            write_payload(&fs, &table_file, block_id, b"guard-root").await;
            ...
            assert!(
                global
                    .invalidate_block(&guard, FileID::from(table_id), block_id)
                    .is_some()
            );
            assert_eq!(test_outstanding_base_guard_count(&global.arena), 1);
        });
    ...
    fn test_readonly_reservation_retries_transient_free_frame_latch_contention() {
        smol::block_on(async {
            ...
            let key = BlockKey::new(test_file_id(8), test_block_id(12));
            ...
            let frame_id = *global
                ...
                .lock()
                .last()
                .expect("readonly test pool must contain one free frame");
            let pool_guard = global.create_base_guard();
            let shared = global
            ...
            let mut reserve = Box::pin(ReadonlyPageReservation::reserve_page(&global, task_arena));

            assert!(futures::poll!(reserve.as_mut()).is_pending());
            {
                let free = global.residency.free.lock();
                assert_eq!(free.len(), free_before);
                assert_eq!(free.last(), Some(&frame_id));
            }

            drop(shared);
            let (reserved_frame_id, page_guard) = reserve.await.unwrap();
            assert_eq!(reserved_frame_id, frame_id);
            let reservation = ReadonlyPageReservation::from_reserved_page(
                global.guard(),
                key,
                reserved_frame_id,
    ...
    fn test_readonly_publish_conflict_panics_and_rolls_back_reservation() {
        smol::block_on(async {
            ...
            let free_before = global.residency.free.lock().len();
            let task_arena = global.arena.arena_guard(global.create_base_guard());
            let (reserved_frame_id, page_guard) =
                ReadonlyPageReservation::reserve_page(&global, task_arena)
                    .await
                    .unwrap();
            assert_ne!(reserved_frame_id, existing_frame_id);
            let reservation = ReadonlyPageReservation::from_reserved_page(
                global.guard(),
                key,
                reserved_frame_id,
            ...
            let panic = catch_unwind(AssertUnwindSafe(|| reservation.publish())).unwrap_err();
            let message = panic
                .downcast_ref::<String>()
                .map(String::as_str)
            ...
            assert!(
                message.contains(&format!("new_frame_id={reserved_frame_id}")),
                "{message}"
            );
    ...
    fn test_readonly_write_barrier_invalidates_resident_mapping() {
        smol::block_on(async {
            ...
            let miss = begin_write_barrier(global.guard(), &pool_guard, key.file_id, key.block_id)
                .unwrap();
            assert!(key_state_is_write_blocked(&global, &key));
            drop(miss);
        });
    ...
    fn test_readonly_write_barrier_rejects_same_key_inflight_load() {
        ...
        assert_eq!(
            ...
            Some(InternalError::ReadonlyWriteInflight)
        );
    ...
    fn test_readonly_read_returns_error_while_write_blocked() {
        smol::block_on(async {
            ...
            let table_file = fs
                ...
                .unwrap();
            let table_file = commit_table_file(&fs, table_file).await;
            let block_id = test_block_id(18);
            write_payload(&fs, &table_file, block_id, b"write-blocked").await;
    ...
    fn test_readonly_write_barrier_survives_cancelled_write_waiter() {
        smol::block_on(async {
            ...
            let table_file = commit_table_file(&fs, table_file).await;
            let block_id = test_block_id(19);
            write_payload(&fs, &table_file, block_id, b"old-write").await;

            let global = global_readonly_pool_scope(frame_page_bytes(2));
            let disk_pool = table_readonly_pool(&global, test_user_table_id(121), &table_file);
            let pool_guard = disk_pool.create_base_guard();
            ...
            let _hook = install_storage_backend_test_hook(write_hook.clone());

            let mut buf = DirectBuf::zeroed(COW_FILE_PAGE_SIZE);
            buf.as_bytes_mut()[..9].copy_from_slice(b"new-write");
            let mutable = MutableTableFile::fork(
                &table_file,
                fs.background_writes(),
                disk_pool.global_pool().clone(),
            ...
            let writer = smol::spawn(async move { mutable.write_block(block_id, buf).await });
    ...
    fn test_read_submission_terminal_completion_is_one_shot() {
        smol::block_on(async {
            ...
            let inflight = Arc::new(PageIOCompletion::new());
            let task_arena = global.arena.arena_guard(global.create_base_guard());
            let (frame_id, page_guard) = ReadonlyPageReservation::reserve_page(&global, task_arena)
                .await
                .unwrap();
            let reservation = ReadonlyPageReservation::from_reserved_page(
                pool.global.clone(),
    ...
    fn test_readonly_cache_file_ids_keep_catalog_and_user_pages_isolated() {
        smol::block_on(async {
            ...
            let catalog_frame_id = publish_test_frame(&global, catalog_key).await;
            let user_frame_id = publish_test_frame(&global, user_key).await;

            assert_ne!(catalog_frame_id, user_frame_id);
            assert_eq!(
                global.try_get_frame_id(&catalog_key),
                Some(catalog_frame_id)
            );
            assert_eq!(global.try_get_frame_id(&user_key), Some(user_frame_id));
        });
    }
    ...
    fn test_global_readonly_pool_panics_on_foreign_guard() {
        smol::block_on(async {
            ...
            let foreign_guard = (*global2).create_base_guard();
            let _ = global1
    ...
    fn test_readonly_pool_reloads_when_mapping_points_to_uninitialized_frame() {
        smol::block_on(async {
            ...
            let stale_frame_id = publish_test_frame(&global, key).await;
            let mut stale_frame = global
                .try_lock_page_exclusive(&global_guard, stale_frame_id)
                .unwrap();
            stale_frame.page_mut().zero();
            let frame = stale_frame.bf_mut();
            frame.clear_persisted_block_key();
            frame.bump_generation();
            frame.set_kind(FrameKind::Uninitialized);
            drop(stale_frame);
    ...
    fn test_readonly_pool_aborts_duplicate_load_after_reservation_when_mapping_exists() {
        smol::block_on(async {
            ...
            let inflight = global_guard
                .join_or_start_inflight_load(table_file.sparse_file(), &pool_guard, key, None)
                .await
    ...
    fn test_readonly_pool_cancelled_single_loader_does_not_leak_completed_inflight() {
        smol::block_on(async {
            ...
            wait_for(|| {
                !global.inflights.contains_key(&key) && global.try_get_frame_id(&key).is_some()
            })
            .await;
    ...
    fn test_readonly_pool_drop_unblocks_detached_reserve_waiter() {
        smol::block_on(async {
            ...
            let task_arena = global_guard
                .arena
                .arena_guard(global_guard.create_base_guard());
            ...
            let reserve_waiter = {
                ...
                let reserve_waiter = smol::spawn(async move {
                    ...
                    match ReadonlyPageReservation::reserve_page(&global, task_arena).await {
    ...
    fn test_readonly_pool_shared_io_failure_propagates_to_all_waiters() {
        smol::block_on(async {
            ...
            let err1 = match waiter1.await {
                Ok(_) => panic!("first readonly waiter should fail"),
                Err(err) => err,
            };
            let err2 = match waiter2.await {
                Ok(_) => panic!("second readonly waiter should fail"),
                Err(err) => err,
            };
