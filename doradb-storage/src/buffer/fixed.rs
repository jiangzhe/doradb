...
mod tests {
    ...
    #[test]
    fn test_facade_page_guard_try_shared_either_refreshes_generation_after_reuse() {
        smol::block_on(async {
            let pool = test_pool();
            let pool_guard = FixedBufferPool::pool_guard(&pool);
            let g = pool.allocate_page::<BlockNode>(&pool_guard).await;
            ...
            pool.deallocate_page(g);
            let g = pool.allocate_page::<BlockNode>(&pool_guard).await;
            let current_generation = g.bf().generation();
            assert_ne!(current_generation, stale_versioned.generation);
            drop(g);

            match stale_guard.try_shared_either() {
                either::Either::Left(_) => {
                    panic!("stale facade guard should not upgrade to shared")
                }
                either::Either::Right(g) => {
                    assert_eq!(g.page_id(), page_id);
                    assert_eq!(g.versioned_page_id().generation, current_generation);
                }
            }
        })
    }
    ...
    #[test]
    fn test_facade_page_guard_try_exclusive_either_refreshes_generation_after_reuse() {
        smol::block_on(async {
            let pool = test_pool();
            let pool_guard = FixedBufferPool::pool_guard(&pool);
            let g = pool.allocate_page::<BlockNode>(&pool_guard).await;
            ...
            match stale_guard.try_exclusive_either() {
                either::Either::Left(_) => {
                    panic!("stale facade guard should not upgrade to exclusive")
                }
                either::Either::Right(g) => {
                    assert_eq!(g.page_id(), page_id);
                    assert_eq!(g.versioned_page_id().generation, current_generation);
