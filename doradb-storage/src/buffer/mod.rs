...
pub trait BufferPool: Sync {
    ...
    /// Get page.
    fn get_page<T>(&self, page_id: PageID, mode: LatchFallbackMode) -> PageGuard<'_, T>;

    /// Deallocate page.
    fn deallocate_page<T: BufferFrameAware>(&self, g: PageExclusiveGuard<'_, T>);
